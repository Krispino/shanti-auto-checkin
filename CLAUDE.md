...
2. No app, conteúdo voltado a hóspedes não traz nome próprio: os contatos são
   `WHATSAPP_SHANTI` (reservas) e `WHATSAPP_GENILDA` (apoio local). A regra vale
   para o produto, não para as conversas — nelas o Fabio se identifica pelo
   nome, e deve: número desconhecido mandando link é o que mais parece golpe.
